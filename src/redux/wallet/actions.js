...
export const userUnlockWallet =
  (meta, accounts, mnemonic) => async (dispatch) => {
...
export const sendTransaction =
  (toAddress, sentAmount, fee, memo, denom, ecpairPriv) =>
  async (dispatch, getState) => {
    return new Promise((resolve, reject) => {
      dispatch(sendTransactionLoading(true));
      const { address } = getState().encrypt;
      fetch(`${SERVER_URL}/users/getAccounts?address=${address}`, {
        method: "GET",
        headers: {
        ...
        .then(async (res) => {
          ...
          const signedTx = await Fincor.sign(stdSignMsg, ecpairPriv);
          ...
          fetch(`${SERVER_URL}/users/webTx`, requestOptions)
