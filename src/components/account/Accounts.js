...
import { ListItemText, Menu, MenuItem, Box, Grid, Container, Typography } from "@material-ui/core";
                ...
                try {
                    Object.entries(changes).forEach(([key, value]) => {
                        if (key !== undefined && key !== null) {
                            const tempArray = changes?.accounts?.newValue;
                            const newAccount = tempArray[tempArray?.length - 1];
                            const data = [
                                {
                                    displayName: newAccount.displayName,
                                    index: activeAccounts[0].index,
                                    address: newAccount.address,
                                },
                            ];

                            const activeItemss = activeAccounts[0].index;
                            dispatch(setAddress(newAccount.address));
                            dispatch(activeIndex(activeItemss));
                            dispatch(activeAccount(data));
                        }
                    });
                } catch (error) {
                    console.log("error account side", error);
                }
            });
        }
    }, [activeAccounts, dispatch]);
    if (loading) {
        return (
