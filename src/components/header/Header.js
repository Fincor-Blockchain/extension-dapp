...
import { AccountCircle, Add, Settings } from "@material-ui/icons";
...
const Header = (props) => {
  ...
  //     console.log("running in logout 2");
  //     return chrome.storage.onChanged.addListener(function () {
  //       dispatch({ type: "SESSION_LOGOUT" });
  //       console.log("running in logout 3");
  //     });
  //   }
  // };
  ...
  // This is for future use for sync tab ///
  useEffect(() => {
    if (ExtensionStore.isSupported) {
      return chrome.storage.onChanged.addListener(function (changes) {
        if (changes.activeAccounts) {
          Object.entries(changes.activeAccounts)?.forEach(([key, value]) => {
