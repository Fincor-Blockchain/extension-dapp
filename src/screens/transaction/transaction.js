...
import { CircularProgress, Box, Grid, Container, Typography } from "@material-ui/core";
...
const Transaction = (props) => {
    ...
    const { coins, allTxs, allTxsLoading, selectedCoin } = useSelector((state) => state.wallet);
    ...
    const getType = (to, from) => {
        const type =
            to === from ? "Self" : address === to ? "Recevied" : address === from ? "Send" : null;
        return type;
    };
    const copyAddress = () => {
        copy(address);
                    ...
                    <Grid item xs={12}>
                        <Box className={classes.main}>
                            ...
                            <Box className={classes.width}>
                                <Box className={classes.titleWrapper} size="small" ref={moreRef}>
                                    <Tooltip
                                        placement="top-end"
                                                    ...
                                                    filterData?.map((item, index) => (
                                                        <a
                                                            href={
                                                                ...
                                                                `${process.env.REACT_APP_EXPLORER_URL}/txs/${item.txhash}`
                                                            }
                                                            target="_blank"
